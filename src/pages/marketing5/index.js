...
const validateURL = (url) => {
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch('/api/marketing5/save-data', {
        method: 'POST',
  ...
  return (
    <div style={{display: "flex", flexDirection: "column", gap: "20px", justifyContent: "center", alignItems: "center"}}>
      ...
      <label style={{backgroundColor: "lightgrey", padding: "5px", borderRadius: "10px"}}>Info: Le QR-Code doit être lié au lien suivant: <b>https://minisite.vocalsms.com/api/marketing5/game-check/</b>
      </label>
...
export async function getServerSideProps() {
  const filePath = path.join('./gameData5.json');
